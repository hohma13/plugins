...
+ (NSString *)saveImageWithMetaData:(NSDictionary *)metaData
    ...
    NSDictionary *newMetadata = [FLTImagePickerMetaDataUtil getMetaDataFromImageData:data];
    if (metaData) {
        NSMutableDictionary *oldMetadata = metaData.mutableCopy;
        if (newMetadata != nil) {
            NSNumber *orientation = newMetadata[@"Orientation"];
            if (orientation.intValue <= UIImageOrientationRightMirrored) {
                [oldMetadata setValue:orientation forKey:@"Orientation"];
                NSMutableDictionary *oldTiff = oldMetadata[@"{TIFF}"];
                if (oldTiff) {
                    [oldTiff setValue:orientation forKey:@"Orientation"];
                    [oldMetadata setValue:oldTiff forKey:@"{TIFF}"];
                }
            }
        }
        data = [FLTImagePickerMetaDataUtil updateMetaData:oldMetadata toImage:data];
    }
